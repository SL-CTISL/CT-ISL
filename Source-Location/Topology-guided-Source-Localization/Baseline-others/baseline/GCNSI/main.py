...
from baseline.GCNSI.model import GCNSI_model
    ...
    def test(self, adj, test_dataset, gcnsi_model, thres, alpha=0.01):
        ...
        with torch.no_grad():
            for influ_mat in test_dataset:
                ...
                test_pr += precision_score(seed_vec, pred >= thres, zero_division=0)
                test_re += recall_score(seed_vec, pred >= thres, zero_division=0)
                test_f1 += f1_score(seed_vec, pred >= thres, zero_division=0)
                test_auc += roc_auc_score(seed_vec, pred)
                
                preds.append(pred >= thres)
