...
if str(_BASELINE_OTHERS_DIR) not in sys.path:
    sys.path.insert(0, str(_BASELINE_OTHERS_DIR))

from baseline.SLVAE.model import VAE, GNN
...
class SLVAE:
    ...
    def train(
        ...
        from utils import load_dataset, diffusion_generation, split_dataset
        ...
        """
        print(f"[train SLVAE]:num_thres{num_thres},lr:{lr},weight_decay:{weight_decay},num_epoch:{num_epoch},print_epoch:{print_epoch},random_seed:{random_seed}")
        ...
        optimizer = Adam(seed_infer, lr=1e-1, weight_decay=weight_decay)

        infer_epoch = 2#int(num_epoch/10)
        ...
        for epoch in range(infer_epoch):
            ...
            if epoch % print_epoch == 0:
                print(f"Epoch [{epoch}/{infer_epoch}], obj = {average_loss:.5f}")
        ...
        for thres in thres_list:
            ...
            print(f"thres = {thres:.5f}, train_f1 = {train_f1:.5f}")
            if train_f1 > opt_f1:
                opt_f1 = train_f1
                opt_thres = thres
        ...
        print(f"Find optimal threshold:{opt_thres:.5f} and F1 score:{opt_f1:.5f}")
    ...
    def infer(
        ...
        """
        print(f"[SLVAE test]:thres:{thres:.5f},lr:{lr},num_epoch:{num_epoch},print_epoch:{print_epoch}")
        ...
        for epoch in range(num_epoch):
            ...
            if epoch % print_epoch == 0:
                print(f"Epoch [{epoch}/{num_epoch}], obj = {average_loss:.5f}")
        ...
        for i, influ_mat in enumerate(test_dataset):
            ...
            # # # 方案1：获取概率前 1% 的值为阈值（top 1%）
            # thres = np.percentile(seed_infer[i].cpu().detach().numpy(), 99.75)  # 获取概率前 1% 的值（即 99 百分位）
