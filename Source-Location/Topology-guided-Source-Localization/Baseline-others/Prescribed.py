...
class LPSI:
    """Label Propagation based Source Identification helper used by GCNSI."""
    ...
    def predict(self, laplacian, num_node, alpha, diff_vec):
        ...
        if not torch.is_tensor(laplacian):
            ...
            laplacian = laplacian.to(self.device).float()
        diff_vec = diff_vec.to(self.device).float()
        inv_matrix = torch.linalg.inv(I - alpha * laplacian)
        return (1 - alpha) * inv_matrix @ diff_vec
...
class NetSleuth:
    ...
    def train(self, adj, train_dataset, k_list=[2, 5, 10]):
        ...
        for influ_mat in train_dataset:
            ...
            f1_scores.append(f1_score(seed_vec, x.cpu().numpy(), zero_division=0))
    ...
    def test(self, adj, test_dataset, k):
        ...
        for influ_mat in test_dataset:
            ...
            metrics['pr'] += precision_score(seed_vec, x.cpu().numpy(), zero_division=0)
            metrics['re'] += recall_score(seed_vec, x.cpu().numpy(), zero_division=0)
            metrics['f1'] += f1_score(seed_vec, x.cpu().numpy(), zero_division=0)
            metrics['auc'] += roc_auc_score(seed_vec, x.cpu().numpy())
            
            pred.append(x.cpu().numpy())

        for key in metrics:
            metrics[key] /= test_num

        return Metric(metrics['acc'], metrics['pr'], metrics['re'], metrics['f1'], metrics['auc']), pred
...
class OJC:
    ...
    def train(self, adj, train_dataset, Y_list=[2, 5, 10]):
        ...
        for influ_mat in train_dataset:
            ...
            f1_scores.append(f1_score(seed_vec, x, zero_division=0))
    ...
    def test(self, adj, test_dataset, Y):
        ...
        for influ_mat in test_dataset:
            ...
            metrics['pr'] += precision_score(seed_vec, x, zero_division=0)
            metrics['re'] += recall_score(seed_vec, x, zero_division=0)
            metrics['f1'] += f1_score(seed_vec, x, zero_division=0)
