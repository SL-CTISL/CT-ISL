...
BASE_RUNNER_PATH = CT_ISL_DIR / "run-CT-ISL-LLM.py"
...
from project_paths import POKEC_LLM_TOPOLOGY_EDGE_PATH, ensure_within_vfsl, rel_to_vfsl, resolve_vfsl_path
...
def _load_base_runner():
    spec = importlib.util.spec_from_file_location("graphsl_llm_base_runner", BASE_RUNNER_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def resolve_checkpoint_path(checkpoint_path):
    ...
    if path.parts and path.parts[0] in {"ckpt&outputs", "llm-gdfsl2025"}:
...
def _topology_root_for(data_name):
    del data_name
    return POKEC_LLM_TOPOLOGY_EDGE_PATH
...
def _candidate_indices(adj, observation, max_nodes, rng):
    ...
    if remaining > 0:
        ...
        pool = np.setdiff1d(np.arange(observation.shape[0], dtype=np.int64), np.fromiter(selected_set, dtype=np.int64))
        if pool.size:
...
def _build_features(profile_embeddings, influ_mat, node_idx):
    profile_idx = torch.as_tensor(node_idx, dtype=torch.long)
    profile = profile_embeddings.index_select(0, profile_idx)[:, :7].detach().cpu().numpy()
    profile = _normalize_columns(_pad_to_dim(profile, 7))

    final_infection = influ_mat[node_idx, 1:2].detach().cpu().numpy()
    infection_context = _pad_to_dim(final_infection, 7)
    return np.concatenate([profile, infection_context], axis=1).astype(np.float32, copy=False)


def _build_records(adj, dataset_subset, profile_embeddings, max_nodes, random_seed):
    ...
    for sample_idx, influ_mat in enumerate(dataset_subset):
        ...
        features = _build_features(profile_embeddings, mat, node_idx)
...
class GDFSL2025:
    ...
    def state_dict(self):
        return self.forward_trainer.state_dict()
    ...
    def train(self, adj, train_dataset_all, profile_embeddings):
    ...
    def test(self, adj, test_dataset_all, profile_embeddings, top_k):
...
def run_experiment(
    ...
    run_root = CKPT_OUTPUT_ROOT / "llm-gdfsl2025" / base_runner._safe_name(data_name)
    ...
    dataset, _ = base_runner.load_dataset(
        data_name=data_name,
        diff_type=diff_type,
        source_ratio=source_ratio,
        topology_id=topology_id,
        topology_root=_topology_root_for(data_name),
        sim_num=sim_num,
    )
    topology = base_runner._topology_name(data_name, topology_id)
    adj, _, _, train_dataset_all, test_dataset_all, train_features, test_features = base_runner.split_dataset(
        dataset,
        train_ratio=0.8,
    )
    ...
    if test_mode:
        ...
        top_k, train_auc, train_f1, _ = model.train(
            adj,
            train_dataset_all,
            train_features["profile_embeddings"],
        )
    ...
    metric, preds = model.test(
        adj,
        test_dataset_all,
        test_features["profile_embeddings"],
        top_k,
    )
    del preds
    ...
    if loaded_checkpoint_path is not None and not trained_new_model:
        ...
        saved_checkpoint_path = base_runner.save_model_checkpoint(
            ...
            metadata={
                ...
                "num_edges": adj.nnz // 2,
                "profile_embedding_path": rel_to_vfsl(base_runner.PROFILE_EMBEDDING_PATH),
                "profile_feature_dim": train_features["profile_embeddings"].shape[-1],
                "topology_snapshot_root": rel_to_vfsl(_topology_root_for(data_name)),
                ...
                "feature_inputs": "GDFSL subgraph runner: observed final-infected nodes plus sampled candidates; first 7 profile embedding dims + final infection status padded to the expected feature width.",
            },
